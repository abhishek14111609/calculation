...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mb-6">
        <form method="GET" action="{{ route('reconciliation.deposits') }}">
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                ...
                <a href="{{ route('reconciliation.deposits.export', request()->query()) }}"
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($deposits as $deposit)
                        <tr class="table-row">
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $deposit->date->format('d M Y') }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-100 to-green-100 flex items-center justify-center">
                                    ...
                                    <span class="font-semibold text-gray-900">{{ $deposit->bank->name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $deposit->source_name ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm font-mono text-gray-500">{{ $deposit->utr ?? '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <span
                                    class="font-bold text-emerald-600 text-base currency">₹{{ number_format($deposit->amount, 2) }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $deposit->remark ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-gray-600 font-medium">No deposits found</p>
